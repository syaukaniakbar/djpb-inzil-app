    ...
    useEffect(() => {
        if (data.start_at && data.end_at) {
            ...
            fetch(`/api/vehicles/available-vehicles?${params}`)
                ...
                .catch(() => setAvailableVehicles(vehicles))
                            ...
                            <div className="space-y-8 md:space-y-12">
                                ...
                                <section>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <select value={data.vehicle_id ?? ''} onChange={(e) => setData('vehicle_id', e.target.value ? Number(e.target.value) : null)} className={`w-full rounded-xl border-gray-200 bg-gray-50/50 px-4 py-3.5 text-sm transition-all focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10 dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:focus:border-blue-500 dark:focus:bg-gray-900 ${errors.vehicle_id ? 'border-red-500' : ''}`} required disabled={loadingVehicles}>
                                            <option value="">{loadingVehicles ? 'Memuat kendaraan...' : 'Pilih Kendaraan...'}</option>
                                            {availableVehicles.map((v) => (
                                                <option key={v.id} value={v.id}>{v.name} — {v.license_plate}</option>
                                            ))}
                                        </select>
                                        {loadingVehicles && <p className="mt-1 text-xs font-medium text-blue-500">Memeriksa ketersediaan kendaraan...</p>}
